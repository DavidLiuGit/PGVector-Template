...
class BaseDocumentManager(ABC):
    ...
    def get_full_corpus(self, corpus_id: str, chunk_delimiter: str = "\n") -> Optional[dict[str, Any]]:
        ...
        chunks = self.session.query(BaseDocument).filter(
            BaseDocument.corpus_id == corpus_id,
            BaseDocument.is_deleted == False
        ).order_by(BaseDocument.chunk_index).all()
        ...
        if full_doc:
            return {
                ...
                "chunks": [{"id": c.id, "index": c.chunk_index, "title": c.title} 
                          for c in chunks if c.chunk_index > 0]
            }
    ...
    def search_by_metadata(self, filters: dict[str, Any], limit: int = 10) -> list[BaseDocument]:
        ...
        query = self.session.query(BaseDocument).filter(
            BaseDocument.is_deleted == False
        )
        ...
        for key, value in filters.items():
            if isinstance(value, list):
                ...
                for nested_key, nested_value in value.items():
                    query = query.filter(text(f"metadata->'{key}'->>'{nested_key}' = :value")).params(value=nested_value)
            else:
                # Simple equality
